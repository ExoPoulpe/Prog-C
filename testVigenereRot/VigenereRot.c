...
unsigned int chiffreRot13(unsigned int Number, char choice[])
{
...
char chiffreVigenere(char texte[],char key[],char stringResult[MAX_SIZE_TEXT],char choice[])
{
    ...
    for(i=0; i < strlen(texte)-1; i++)
    {
       ...
       // valueDechiffrer=Cut((locate(texte[i]) - locate(Key[j])-1));
        ...
        if((choice[0]=='D' || choice[0] == 'd') && (choice[1] =='R' || choice[1] == 'r'))
        {
            if(texte[i]!=' ')
            {
                valueText = chiffreRot13(valueText,choice);
                valueResult=carOverFlowList(valueText-valueKey);
                stringResult[i] = lettre[carOverFlowList(chiffreRot13(valueResult,choice))];//=  %d\n
            }
            else if(texte[i]== ' ')
            {
                stringResult[i] = ' ';
...
int main()
{
    ...
    printf("\nChiffrement de vigenere (h = Help) >");
    //scanf("%c",&Choice);
    ...
    //scanf("%s",Text);
    //fgets(Text,MAX_SIZE_TEXT,stdin);
    chiffreVigenere(Text,Key,result,&Choice);
    printf("%s",result);
    }
    else if(Choice[0]=='D' || Choice[0] == 'd')
    {
    ...
    {
        puts("\nBienvenu dans se systeme de chiffrement basique pour vigenere\n");
        puts("\nvous pouvez chiffrer a l'aide de : \n - vigenere\n - vigenere + cezar \net vous pouvez bien sur les dechiffrer");
